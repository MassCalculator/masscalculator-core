...
TEST_F(BrassTest, GetSpecificTypeTest)
{
  std::pair<std::string, MassCalculator::Brass::Type> 
    type{"B_240Low", MassCalculator::Brass::Type::B_240Low};
...
{
  EXPECT_EQ( double{2.71}, material_B_240Low.getSpecificDensity());
}

TEST_F(BrassTest, GetSpecificGravityTest)
{
  EXPECT_EQ( double{2.83}, material_B_240Low.getSpecificGravity());
}

TEST_F(BrassTest, GetSpecificMeltingPointTest)
{
  EXPECT_EQ( double{537.778}, material_B_240Low.getSpecificMeltingPoint());
}
...
{
  EXPECT_EQ( double{990.0}, material_B_240Low.getSpecificThermalConductivity());
}

TEST_F(BrassTest, GetSpecificModOfElasticityTensionTest)
{
  EXPECT_EQ( double{9.9}, material_B_240Low.getSpecificModOfElasticityTension());
}

TEST_F(BrassTest, GetSpecificModOfElasticityTorsionTest)
{
  EXPECT_EQ( double{3.8}, material_B_240Low.getSpecificModOfElasticityTorsion());
}
