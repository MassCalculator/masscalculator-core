#include "shapes/octagon_bar.hpp"

namespace MassCalculator
{

  OctagonBar::OctagonBar(double side, double length)
  {
    std::cout << "From constructor: " << side * length << std::endl;
  }

  bool OctagonBar::setSize(double side, double length)
  {
    std::cout << "I just set the size of OctagonBar\n"
  ...
  //TODO:
  double OctagonBar::getDiagonal(void) const
  {
    return this->properties_.side_/2;
  }

  double OctagonBar::getSide(void) const
  {
    return this->properties_.side_;
  }

  double OctagonBar::getLength(void) const
  {
    return this->properties_.length_;
  }

  //TODO:
  double OctagonBar::getVolume(void) const
  {
  ...
  double OctagonBar::getSurfaceArea(void) const
  {
    //2	π	r	 (	r	+	h	)
    return (2 * M_PI * this->getSide() * (this->getSide() + this->getLength()));
  }

  std::string OctagonBar::getType(void) const
  {
    return this->properties_.type_.first;
  }

  std::ostream &operator << (std::ostream &os, const OctagonBar &obj)
  {
    os << "  OctagonBar object properties: " "\n"
