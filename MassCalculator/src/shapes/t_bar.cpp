...
namespace MassCalculator::Shapes
{
  ...
  bool TBar::setSize(double side, double length)
  {
    std::cout << "I just set the size of TBar\n"
  ...
  //TODO:
  double TBar::getDiagonal(void) const
  {
    return this->properties_.side_/2;
  }

  double TBar::getSide(void) const
  {
    return this->properties_.side_;
  }

  double TBar::getLength(void) const
  {
    return this->properties_.length_;
  }

  //TODO:
  double TBar::getVolume(void) const
  {
  ...
  double TBar::getSurfaceArea(void) const
  {
    //2	π	r	 (	r	+	h	)
    return (2 * M_PI * this->getSide() * (this->getSide() + this->getLength()));
  }

  std::string TBar::getType(void) const
  {
    return this->properties_.type_.first;
  }

  std::ostream &operator << (std::ostream &os, const TBar &obj)
  {
    os << "  TBar object properties: " "\n"
